...
try {
const file = path.join(_dirname, "../../data/session_config" + sessionId + ".json");
...
try {
const dataFolder = path.join(_dirname, "../../data");
if (!existsSync(dataFolder)) mkdirSync(dataFolder, { recursive: true });
const file = path.join(dataFolder, "session_config" + sessionId + ".json");
...
{footer}; const safeName = (info.title + (${file.quality}).mp4).replace(/[/\:*?"<>|]/g, "");
...
``${API_BASE}/cinesubz-search?q= {API_KEY}`
);
...
let infoText = 🎬 *${info.title}*\n\n; if (info.year) infoText +=📅 Year:latex
{info.year}\n`; if (info.rating) infoText += `⭐ *Rating:* 

{info.rating}\n; if (info.duration) infoText +=⏱️ Duration:latex
{info.duration}\n`; if (info.country) infoText += `🌍 *Country:* 
